...
$query = "SELECT * FROM postproduct WHERE sale_status = 'On sale' ORDER BY post_id DESC ";
	...
	foreach ($result as $row) 
	{
		...
		$output .= '
			<tr>
				...
				<td id="status">'.$row["sale_status"].'</td>
				<td> &#8369; '.number_format($row["price"],2).'</td>
				<td> &#8369; '.number_format($row["new_price"],2).'</td>
				<td>
				<button type="button" name="edit" class="btn btn-primary " id="'.$row["post_id"].'" onclick="sellSwine(this)"><i class="fa fa-eye"></i> view</button></td>
			</tr>
		';
	}
}
