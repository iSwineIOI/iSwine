...
<head>
  ...
  <section id="container">
              ...
              <ul class="sidebar-menu" id="nav-accordion">
                  ...
                  <li id="content-desktop" class="sub-menu">
                      ...
                      <ul class="sub">
                          ...
                          <li class="active"><a  href="sold.php">Sold</a></li>
                      </ul>
                  </li>
                  <li id="content-desktop" class="sub-menu">
                      <a href="javascript:;" >
      ...
      <!--main content end-->
       <!-- Modal for mail -->
          <div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="addToSale" class="modal fade">
              <div class="modal-dialog modal-sm">
                  <div class="modal-content">
                      <div class="modal-header">
                          ID: <Label class="modal-title" id="pId"></Label><hr>
                          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                              <span aria-hidden="true">&times;</span>
                          </button>
                      </div>
                      <div class="modal-body">
                      <label>Set a new price:</label>
                        <div class="row">
                            <div class="col-lg-6">
                            <label>Current Price:</label>
                            <input type="number" class="form-control" id="currentprice" name="" readonly>
                            </div>
                            <div class="col-lg-6">
                            <label>New Price:</label>
                            <input type="number" class="form-control" id="newprice" name="">   
                            </div>
                        </div>
                      </div>
                      <div class="modal-footer">
                          <button class="btn btn-primary postID"  id="postid" onclick="addToSale(this)" type="button"><i class=" fa fa-plus"></i> Add to sale</button>
                      </div>
                  </div>
              </div>
          </div>
          ...
          <script type="text/javascript">
              function saleSwine(button){
                var post_id = button.id;
                //var identify = 'fetchPrice';
                //alert(post_id);
                $.ajax({
                    url: 'saleProduct.php',
                    method: 'post',
                    ...
                    {
                        document.getElementById('postid').id = data.post_id;
                        $("#pId").html(data.post_id);
              ...
              function addToSale(button){
                var postNew_id = button.id;
                var new_price = $("#newprice").val();
                //alert(postNew_id);
                $.ajax({
                    url: 'AddtoSale.php',
                    method: 'post',
                    data:{'postNew_id':postNew_id,'new_price':new_price},
                    success:function(data)
                    {
                        alert(data);
                        toastr.success("Item is now on sale","Successful");
                        load_data();
                    }
                })
              }
          ...
          <!--modal start-->
                  <!-- Modal -->
              <div class="modal fade" id="swineSellCalc" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                  <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                      <div class="modal-content">
                          <div class="modal-header">
                              <h5 class="modal-title" id="exampleModalCenterTitle">Set selling price | <small>ID: </small><small id="Sow_IDisplay"></small></h5>
                              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                  <span aria-hidden="true">&times;</span>
                              </button>
                          </div>
                            ...
                            <div class="col-lg-3">
                                <div class="form-group ic-cmp-int">
                                    ...
                                    <div class="form-group">
                                        <input type="text" style="text-align: center;" id="product_name" name="product_name" class="form-control" placeholder="Product name" >
...
<!-- /.modal -->
  ...
  <script type="text/javascript">
    function viewPurchase(button){
      var purchase_code = button.id;
      //alert(purchase_code);
        $.ajax({
        url: 'getPurchaseInfo.php',
        method: 'post',
        data:{'purchase_code':purchase_code},
        success:function(data)
        {
            $("#purchaseDetail").html(data);
            $("#viewMore").modal("show");
        }
      })
    }
    ...
    $.ajax({
      ...
      method: 'post',
      data:{'post_id':post_id,'purchase_code':purchase_code},
      success:function(data)
      {
         $.ajax({
          url: 'getPurchaseInfo.php',
    ...
    function confirmAll(button){
    var trans_id = button.id;
    //alert(trans_id);
    $.ajax({
      url: 'recivePayment.php',
      method: 'post',
  ...
  <script type="text/javascript">
      function sellSwine(button){
        ...
        $.ajax({
            ...
            {
                //alert(data);
                window.location = "product_datails.php?swineID="+data;
            }
        })
      }
  </script>
